...
case $localflag in
  0)
    ...
    echo "test start: mv";owncloud.sh  mv testdir/test.txt test.txt
    echo "test start: get";owncloud.sh  get test.txt
    echo "data:\n>  ";cat test.txt
    if [ $cache -ne 1 ]
    then
      echo "test start: rm";owncloud.sh rm testdir;owncloud.sh rm test.txt
